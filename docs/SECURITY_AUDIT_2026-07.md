...
| **F13** snapshots not append-only | ✅ **Fixed & verified live (2026-07-23)** | [`0017`](../WebApp/db/migrations/0017_snapshots_append_only.sql) applied — contributor+ gate on snapshot/revision writes (0005 stubbed this and never revisited it) plus a `BEFORE UPDATE` trigger so even the service key cannot restate the historical quantities the 5D/6D revision diff is computed from. Verified live: counts unchanged (37,525 / 19), an in-place UPDATE is rejected with the append-only error **even for the elevated role**, and the FK cascade still removes child snapshots when a revision is deleted (both tests rolled back). **Deliberate deviation:** DELETE stays with the FK cascade — a blanket guard would make deleting a project or superseded revision fail, and snapshots are derived data, not evidence like `audit_log`. |
| **Advisors** anon-callable helpers | ✅ **Fixed & verified live (2026-07-23)** | [`0018`](../WebApp/db/migrations/0018_revoke_anon_execute_helpers.sql) — `anon` kept EXECUTE on `is_member` / `has_min_role` / `project_of_container` via the default PUBLIC grant. `project_of_container` was the real leak: it returns the owning project for any container id, so an unauthenticated caller could map containers to projects. Revoked from `anon`+`public`, granted explicitly to `authenticated`. The three Supabase advisor WARNs cleared; `security:check` and 99 tests still pass. |
